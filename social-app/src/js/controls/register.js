...
export function RegisterPage() {
    ...
    function submitHandlerRegistration(e) {
        ...
        auth.register(regEmailInput.value, regPasswordInput.value, nickNameInput.value, firstNsmeInput.value, lastNameInput.value, phoneInput.value, genderInput.value, cityInput.value, countryInput.value, dayOfBirthInput.value, monthOfBirthInput.value, yearOfBirthInput.value)
            .then((res) => {
                if (!res.error) {
                    formReg.reset();
